...
class Auth {
    async login(req, res, next) {
        try {
            ...
            const pubUser = { username: user.username, email: user.email, role: user.role }
    ...
    async register(req, res, next) {
        try {
            ...
            const pubUser = { username: savedUser.username, email: savedUser.email, role: savedUser.role }
            const token = jwt.sign(pubUser, JWT_SECRET)
            res.json({ user: pubUser, token })
        } catch (error) {
            next(error)
